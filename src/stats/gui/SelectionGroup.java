...
/**
 * Represents very small size.
 */
public final static int SIZE_VERYSMALL = 1;
...
/**
 * Represents very large size.
 */
public final static int SIZE_VERYLARGE = 5;
...
public SelectionGroup(String name) {
  this(name, MARKER_FULL_CIRCLE, Color.BLACK, SIZE_VERYSMALL);
}

/**
 * Constructor for {@code SelectionGroup} class given group name, marker type,
 * color and size. All properties of the object are directly initialized. If any
 * argument is not valid, default initialization is used.
 *
 * @param name name of the selection group.
 ...
 * selection group. The marker type argument must be defined as static member of
 ...
 * index of a {@code Table} object. It must be a positive integer number. If the
 * element is already present in the selection group, the function will return
 * false.
 *
...
/**
 * Removes an element from the selection group. The element is commonly the row
